...
// Cloudflare gateway — no rate limits unlike public Pinata gateway.
...
export function isPhaseActive(
  ...
  if (!active) return { status: 'ended', label: 'Inactive' };
  if (now < startTime) return { status: 'upcoming', label: `Starts ${formatDate(startTime)}` };
  if (endTime > 0n && now > endTime) return { status: 'ended', label: 'Ended' };
