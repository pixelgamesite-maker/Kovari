...
import { toGateway } from "@/lib/utils";
...
export function CollectionInfoEditor({ collection }: { collection: Address }) {
  ...
  useEffect(() => {
    if (metadata && !initialized) {
      ...
      if (metadata.image) setLogoPreview(toGateway(metadata.image) ?? null);
      if (metadata.banner) setBannerPreview(toGateway(metadata.banner) ?? null);
      setInitialized(true);
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-main-text focus:border-accent-blue/50 focus:outline-none resize-none"
        />
      </div>

      <div>
        <label className="mb-1.5 block text-sm font-medium text-main-text">External link</label>
        <input type="url" value={externalLink}
      ...
      {isConfirmed && <p className="text-sm text-green-400">Collection info updated.</p>}
      ...
      >
        {isUploading ? <><Loader2 size={16} className="animate-spin" /> Uploading images...</>
          : isPending ? <><Loader2 size={16} className="animate-spin" /> Confirm in wallet...</>
          : isConfirming ? <><Loader2 size={16} className="animate-spin" /> Saving...</>
